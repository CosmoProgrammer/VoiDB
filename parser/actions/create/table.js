    ...
    if (!exceptionTypes.includes(expectedType)) {
      if (!(expectedType === actualType)) {
        return [
          ...
          new classes.Error(
            "TypeError",
            `Default Values don't match with Types. Type of ${values[x]} is ${actualType} and not ${expectedType}`
          ),
        ];
      }
    } else {
...
function createTable(code, preRunData) {
  ...
  if (fs.existsSync(`${storagePath}\\${USING_DATABASE}\\${table.name}.json`)) {
    ...
    if (preRunData[0])
      return new classes.Success("Created a table with name successfully");
  }
}
