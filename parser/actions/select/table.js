...
function readValues(code, preRunData, table) {
  ...
  return new classes.Data(
    finalResult,
    `This is the requested values from the table ${table.name}`
  );
}
...
function readValuesWhere(code, preRunData, table) {
  ...
  let whereConditionParsed = whereConditionUnparsed
    .replace("||", "splithere")
    .replace("&&", "splithere")
    .split(" splithere ");
  let whereCondition = [];
  ...
  for (let x in finalResult) {
    if (
      checkIfRowIsToBeRemoved(finalResult[x], whereCondition, logicalOperator)
    ) {
      finalResult[x] = null;
    }
  }
...
function readValuesOrder(code, preRunData, table) {
  ...
  if (ascdesc === "asc" || ascdesc === "ascending") {
    eval(
      `finalResult = finalResult.sort((a,b)=> (a['${order}'] > b['${order}'] ? 1 : -1))`
    );
...
function checkIfRowIsToBeRemoved(row, whereConditions, loperator) {
  ...
  for (let i in whereConditions) {
    conditions.push(
      `('${row[whereConditions[i]["column"]]}' ${
        whereConditions[i]["operater"]
